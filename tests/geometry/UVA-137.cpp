...
signed main() {
    ...
    while (true) {
        ...
        rep(i,0,m){
            ls.pb(make_pair(q[i],q[(i+1)%m]));
        }

        cout << fixed << setprecision(2);
        D ans = (area(p)+area(q))*0.5-area(halfplaneIntersection(ls));
        int res = floor(ans * 100);
        ...
        cout << (res/100)%10 << "." << (res/10)%10 << res%10;
    }
	return 0;
}
