...
signed main() {
    ...
    vector<L>ls;
    rep(i,0,n){
        P d=unit(rot90(p[(i+1)%n]-p[i]))*R;
        ls.pb(make_pair(p[i]+d,p[(i+1)%n]+d));
    }
    vector<P> hi=halfplaneIntersection(ls);

    pair<ld,pair<P,P>> res=make_pair((ld)0,make_pair(xxd(0,0),xxd(0,0)));
    rep(i,0,sz(hi))rep(j,0,sz(hi)){
        res=max(res,
            make_pair(len(hi[i]-hi[j]),
            make_pair(hi[i],hi[j])));
    }
